########################################
# Graph recovery simulation for Case 1 #
...
create_adj_cov <- function(n,p,r)
{
  ...
  X_mat <- mvrnorm(n,mu=rep(0,p),Sigma = cov_mat)
...
plot(perf,col="red",lty=1, lwd=2, avg="threshold", main="Case 1", xlab="False positive rate",ylab="True positive rate")
plot(perf2,lty=2, lwd=2, avg="threshold",add=TRUE)
plot(perf3,lty=2, lwd=2, add=TRUE, avg="threshold",col="green")
legend("bottomright", c("scaled-Ecov", "GGM","TGM"), col = c("red", 1,"green"), lty = c(1,2,2))
