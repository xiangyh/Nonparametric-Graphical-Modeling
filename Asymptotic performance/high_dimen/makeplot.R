...
size_path <- "/Users/yunhuaxiang/Desktop/project/graph_recovery/simulation_no_bs/Psi1/high_dim/high_dimen9/sim"

iter <- 20
Onestep <- Onestep_CI <- Naive <- NULL
size <- c(500, 1000, 2000, 3000, 4000, 5000, 6000)


for(i in 1:iter){
  fn <- paste(size_path,i,"/Onestep.txt", sep = "")
  Onestep <- rbind(Onestep,read.table(fn))
}

for(i in 1:iter){
  fn <- paste(size_path,i,"/Onestep_CI.txt", sep = "")
  Onestep_CI <- rbind(Onestep_CI,read.table(fn))
}

for(i in 1:iter){
  fn <- paste(size_path,i,"/Naive.txt", sep = "")
  Naive <- rbind(Naive,read.table(fn))
}
...
Cover.ggplot <- function(plt, main="")
{
  plt + 
    ...
    geom_point(aes(shape = Est, color = Est), position=position_dodge(100), size=2.5) + # 21 is filled circle
    #scale_color_manual(values = c("red","blue","green"))+
    scale_color_manual(values = "blue")+
    xlab("Size") +
    #expand_limits(y=0) +                        # Expand y range
    scale_y_continuous(breaks=seq(0,1,by=0.2),limits = c(0,1)) +         # Set tick every 4
    theme_bw() +
    labs(title=main)+
...
mylegend<-g_legend(Bias.plt)


pdf("/Users/yunhuaxiang/Desktop/project/graph_recovery/simulation_no_bs/Psi1/high_dim/high_dimen9/Psi1_high.pdf", height = 3, width = 5.5)
grid.arrange(arrangeGrob(Bias.plt+theme(legend.position="none"), 
                         cover.plt+theme(legend.position="none"), nrow = 1), 
             mylegend, nrow=2,heights=c(10, 1),
             top = textGrob("High-dimensional setting",gp=gpar(fontsize=15), vjust = 1.3))
dev.off()
